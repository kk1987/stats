...
public final class HistoryRecorder {
    ...
    /// #3450's per-day byte counter. Not a lane and not a tier: its own small
    /// file beside the archives, fed from the same per-tick deltas the Net
    /// lanes are and rolled at local midnight (§5).
    private let daily: HistoryDailyTraffic
    ...
    public init(store: HistoryStore, preset: HistoryRetentionPreset = .standard,
        ...
        // The calendar is the daily counter's only environment beyond the
        // clock the rest of the recorder already injects: a test that has to
        // put a DST transition or a flight to another time zone under it
        // cannot do either by moving the clock alone (§5).
        self.daily = HistoryDailyTraffic(
            url: store.directory.appendingPathComponent(HistoryDailyTraffic.fileName),
            calendar: calendar
        )
    ...
    ///
    /// The daily traffic counter is deliberately not one of them: it is not a
    /// lane, has no slot in any tier and costs no lane budget, so it is
    /// excluded from every lane count and size figure the design quotes (§1).
    /// Its file is inside the directory `bytesOnDisk` sums, which is where a
    /// user should see the ~10 KiB it actually costs.
    public var laneCount: Int { self.locked { self.directory.count } }

    /// Today's and yesterday's network bytes, rolled to now (§5,
    /// exelban/stats#3450). What the Net popup shows, read straight off the
    /// in-memory counter: no file is touched and nothing blocks.
    ///
    /// The figures are what *this* app counted while it was recording, so they
    /// read below both the ISP's number and the Net popup's own total two rows
    /// above: sleep, the time Stats was not running and the reader's own
    /// substituted zeros are all missing from them, and the row says so.
    public var dailyTraffic: HistoryDailyTraffic.Totals {
        self.daily.totals(at: self.environment.now())
    }
    ...
    public func stop() {
        ...
        self.queue.sync {
            ...
            self.commit(at: now)
            // Forced, like the one in `flush`: a stop is the end of this
            // process's contribution to the day, and the counter's last few
            // minutes have nowhere else to go.
            self.daily.commit(at: now, force: true)
            self.store.sync()
    ...
    public func setPreset(_ preset: HistoryRetentionPreset) -> PresetChange {
        ...
        self.queue.sync {
            ...
            for tier in dropped.sorted(by: { $0.rawValue < $1.rawValue }) {
                ...
                guard !self.store.removeTier(tier) else { continue }
                outcome = .partial
            }
    ...
    public func deleteAll() -> Bool {
        ...
        self.queue.sync {
            ...
            // back out on the next sleep. The daily counter goes the same way:
            // its file was in the directory that was just emptied, and a
            // counter that kept its total would write yesterday's bytes back
            // into a history the user has just deleted.
            self.sleepMonitor.load()
            self.daily.reset(at: self.environment.now())
    ...
    public func flush() {
        self.queue.sync {
            let now = self.environment.now()
            self.commit(at: now)
            // Forced, unlike the commit tick's: this is the moment the periodic
            // cadence cannot cover, and the counter's last few minutes are
            // exactly what a terminate would otherwise drop.
            self.daily.commit(at: now, force: true)
            self.store.sync()
            self.lastSyncTs = now
        }
    ...
    private func performStart(enabled: Bool) {
        ...
        // The daily counter picks today's total back up out of its own file, so
        // a relaunch at noon continues the morning's figure instead of starting
        // the day again.
        self.daily.load(at: now)
    ...
    public func ingest<T>(_ value: T, reader: @autoclosure () -> HistoryReaderKey,
        ...
        self.locked {
            ...
            // #3450's counter is fed here, from the payload's own per-tick byte
            // delta, before anything is bucketed or averaged. Integer adds
            // under a leaf lock — the order is this lock, then the counter's,
            // and nothing takes them the other way round.
            let traffic = self.sink.traffic
            if !traffic.isEmpty { self.daily.add(traffic, at: now) }
            guard !self.sink.isEmpty || !traffic.isEmpty else { return }

            // Traffic alone still wants the timer: at the 256-lane cap the Net
            // lanes can be refused while the counter goes on accruing, and a
            // parked timer would then leave it unpersisted until something
            // else committed.
            if !self.sink.isEmpty { self.table.fold(self.sink, at: now, interval: interval ?? 0) }
    ...
    private func commit(at now: TimeInterval) {
        ...
        // The daily counter rides this tick and no timer of its own (§5). It
        // rolls the day on every cycle — the calendar is asked again rather
        // than a midnight cached at launch — and writes only when its own
        // cadence is due, which is what keeps a two-number counter from
        // dirtying a block every minute.
        self.daily.commit(at: now)
    ...
    public func noteWillSleep(at ts: TimeInterval) {
        ...
        self.queue.sync {
            self.commit(at: ts)
            self.daily.commit(at: ts, force: true)
            self.store.sync()
            self.lastSyncTs = ts
            self.parkTimerForGood()
        }
